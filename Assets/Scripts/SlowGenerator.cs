...
public class SlowGenerator : MonoBehaviour {
	...
	float worldUnitsInOneGridCell = 2.5f;
	...
	IEnumerator FloorLoop(){
        int rand = Random.Range(0, floorObj.Length);
		...
		do{
			...
			foreach (walker myWalker in walkers){
				if (grid[(int)myWalker.pos.x,(int)myWalker.pos.y] != gridSpace.floor){
					Spawn(myWalker.pos.x,myWalker.pos.y, floorObj[rand]);//update visuals
					spawned = true;
				}
		...
		for (int x = 0; x < roomWidth-1; x++){
			for (int y = 0; y < roomHeight-1; y++){
				...
				if (grid[x,y] == gridSpace.floor){
					...
					if (placed){
						yield return new WaitForSeconds(timeBetweenLoops/1);
					}
				}
			}
		}
	...
	void Spawn(float x, float y, GameObject toSpawn){
        ...
        obj.transform.parent = GameObject.Find("Generated Level").transform;
    }
}
